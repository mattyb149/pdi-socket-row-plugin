...
package org.pentaho.di.trans.steps.socketrowcollector;
...
/**
 * Write data to a TCP/IP socket read by SocketRowProducer. The data being sent over the socket is one serialized Row object
 * including metadata and then a series of serialized rows, data only.
 *
 * This part of the SocketRowCollector/SocketRead pair contains the ServerSocket.
 *
 * @author Matt
 * @since 1-dec-2006
 */
public class SocketRowCollector extends BaseStep implements StepInterface {
  private static Class<?> PKG = SocketRowCollectorMeta.class; // for i18n purposes, needed by Translator2!!

  private SocketRowCollectorMeta meta;
  private SocketRowCollectorData data;

  public SocketRowCollector( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
    Trans trans ) {
    super( stepMeta, stepDataInterface, copyNr, transMeta, trans );
  }

  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    meta = (SocketRowCollectorMeta) smi;
    data = (SocketRowCollectorData) sdi;
    ...
    try {
      ...
      if ( data.clientSocket != null ) {
        try {
          ...
          logError( "Closed connection to SocketRowCollector" );
        } catch ( IOException e1 ) {
          logError( "Failed to close connection to SocketRowCollector" );
        }
      }
    ...
    if ( checkFeedback( getLinesRead() ) ) {
      logBasic( BaseMessages.getString( PKG, "SocketRowCollector.Log.LineNumber" ) + getLinesRead() );
    }
