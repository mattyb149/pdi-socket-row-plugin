...
/**
 * Read data from a TCP/IP socket supplied by SocketRowCollector. The data coming over the socket is one serialized Row object
 * including metadata and then a series of serialized rows, data only.
 *
 * @author Matt
 * @since 01-dec-2006
 */
public class SocketRowProducer extends BaseStep implements StepInterface {
  ...
  private SocketRowProducerMeta meta;
  private SocketRowProducerData data;

  public SocketRowProducer( StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
    Trans trans ) {
    super( stepMeta, stepDataInterface, copyNr, transMeta, trans );
  }

  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    meta = (SocketRowProducerMeta) smi;
    data = (SocketRowProducerData) sdi;
    ...
    try {
      ...
      if ( first ) {
        ...
        if ( lastException != null ) {
          ...
          if ( data.inputStream == null ) {
            throw new KettleException( "Unable to connect to the SocketRowCollector in the "
      ...
      if ( checkFeedback( getLinesInput() ) ) {
        logBasic( BaseMessages.getString( PKG, "SocketRowProducer.Log.LineNumber" ) + getLinesInput() );
      }
